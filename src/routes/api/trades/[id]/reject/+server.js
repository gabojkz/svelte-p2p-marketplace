...
export async function PATCH({ params, locals }) {
  ...
  // The other party (seller) should reject
  if (trade.buyerId === marketplaceUser.id) {
    throw error(400, "You cannot reject your own trade initiation");
  }
  ...
  await db
    ...
    .set({
      ...
      cancellationReason: "Trade rejected by seller",
      updatedAt: new Date(),
    })
    .where(eq(trades.id, tradeId));
  ...
  // Create system message about trade rejection
  if (conversation) {
    ...
    const userName = rejectingUser?.firstName || rejectingUser?.username || "User";
    
    // Create system message visible to both parties
    await db
      .insert(messages)
      .values({
        conversationId: conversation.id,
        senderId: marketplaceUser.id,
        content: `${userName} has rejected the trade.`,
        isRead: false,
      });
    ...
    await db
      ...
      .set({
        ...
        lastMessagePreview: `${userName} has rejected the trade.`,
  ...
  return json({ success: true, message: "Trade rejected successfully" });
}
