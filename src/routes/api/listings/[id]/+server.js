...
import { listings, users, categories, listingImages } from '$lib/server/schema';
...
export async function PUT({ params, request, locals }) {
	...
	if (existingListing.userId !== marketplaceUser[0].id) {
		throw error(403, 'Access denied');
	}
	...
	try {
		...
		const [updatedListing] = await db
			...
			.set({
				...
				publishedAt: body.status === 'active' && !existingListing.publishedAt ? new Date() : existingListing.publishedAt,
		...
		return json({ success: true, listing: updatedListing });
	} catch (err) {
